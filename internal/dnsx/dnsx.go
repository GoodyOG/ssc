...
type Config struct {
	...
	TimeoutSec int      // DNS query timeout in seconds
}
...
func ResolveHost(ctx context.Context, cfg Config, host string) ([]string, string) {
	...
	if len(servers) > 0 {
		if ips := resolveViaDirectDNS(ctx, host, servers, timeout); len(ips) > 0 {
			cache(cacheKey, ips, method)
			return ips, method
		}
	}

	if ips := shellResolve(ctx, host, timeout); len(ips) > 0 {
...
func EvictHost(host string) {
	...
	if evicted > 0 {
		log.Printf("dnsx: cache evicted %d entries for %s", evicted, host)
	}
}
...
func AndroidCarrierDNS(ctx context.Context) []string {
	...
	for _, prop := range props {
		...
		// One working DNS server is enough to proceed. Stop scanning to
		// avoid spending up to 16 × 300ms on unused getprop keys.
		break
	}
...
// resolveViaDirectDNS sends an A query to all servers simultaneously and
// returns the first successful result. Fanning out avoids the sequential
// 300ms-per-server timeout that caused slow first-connects when early
// servers were unreachable (common on some Android carrier configurations).
func resolveViaDirectDNS(ctx context.Context, host string, servers []string, timeoutSec int) []string {
	type result struct{ ips []string }
...
func shellResolve(ctx context.Context, host string, timeoutSec int) []string {
	cmds := [][]string{
