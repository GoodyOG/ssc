#!/system/bin/sh
# service.sh - SSHCustom v2.7.0 boot handler
#
# Boot flow:
...
wait_api_ready() {
  for i in 1 2 3 4 5 6 7 8; do
    if command -v curl >/dev/null 2>&1; then
      curl -fsS --max-time 1 "http://127.0.0.1:9190/api/v1/health" >/dev/null 2>&1 && return 0
    elif command -v wget >/dev/null 2>&1; then
      wget -q -T 1 -O /dev/null "http://127.0.0.1:9190/api/v1/health" >/dev/null 2>&1 && return 0
    fi
    sleep 1
  done
  return 1
